...
-- Merchants users want tracked / suggested to grow the catalog (name + location hints)
CREATE TABLE user_merchant_suggestions (
    id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_user_merchant_suggestions_user_id ON user_merchant_suggestions(user_id);
